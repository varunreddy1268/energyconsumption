#@author: varun reddy seelam
import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
import seaborn as sns
import tensorflow as tf
from tensorflow.keras.layers import Input, Conv1D, BatchNormalization, Dense, Dropout,LSTM
...
class model_building:
    def __init__(self,input_shape,number_of_layers_you_want):
    ...
    def create_model(self):
        ...
        self.model=LSTM(units=120,input_shape=self.input_shape,return_sequences=True)(self.input)
        ...
        for i in range(self.number_of_layers_you_want):
            #This adds the LSTM layers to the existing model
            self.model=LSTM(units=120,return_sequences=True)(self.model)
        ...
        self.model=LSTM(units=120,return_sequences=False)(self.model)
        #The final touch with the dropout
        self.model=Dropout(0.2)(self.model)
        #Lastly the output node luckily we have one target class so go with that
        self.output = Dense(units=1, activation = 'tanh')(self.model)
