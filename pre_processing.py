...
class preprocessing:
    def __init__(self,meta_data,main_column,n_train_days,n_test_days,n_sample_minutes):
    ...
    def data_frame_convertor(self):
        self.meta_data=self.meta_data[["date","active_energy"]]
    ...
    def train_main_convertor(self):
        ...
        for i in range(start,self.trainer.shape[0]):
            self.meta_train.append(self.trainer[i-start:i])
            self.meta_target.append(self.trainer[i])
    ...
    def test_main_convertor(self):
        ...
        for i in range(start,self.tester.shape[0]):
            self.meta_test.append(self.tester[i-start:i])
            self.meta_test_target.append(self.tester[i])
        #Now the value's shape that are there in the meta_train was(60,)
        #For the target the shape is 1
    ...
    def supreme_method(self):
        ...
        self.meta_train,self.meta_target,self.meta_test,self.meta_test_target=self.array_convertor()
        #Basically,
